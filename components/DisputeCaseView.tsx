...
export default function DisputeCaseView({ disputeId }: DisputeCaseViewProps) {
  ...
  if (loading) {
    return (
      <GlassCard>
        <div className="text-[#86868b] font-light text-center py-12">Loading case...</div>
      </GlassCard>
    )
  }
  ...
  if (!caseData) {
    return (
      <GlassCard>
        <div className="text-[#86868b] font-light text-center py-12">Case not found</div>
      </GlassCard>
    )
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <GlassCard className="border-blue-500/30 bg-blue-500/5">
          <div className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-light text-[#1d1d1f]">AI Analysis</h2>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-[#86868b] font-light">Suggested Outcome</span>
                ...
                <p className="text-gray-700 font-light text-sm">{aiAnalysis.reasoning}</p>
                <div className="mt-2">
                  <span className="text-xs text-[#86868b]">Confidence: </span>
                  <span className="text-xs text-gray-700 font-medium">{aiAnalysis.confidenceScore}%</span>
                </div>
              </div>
              ...
              {aiAnalysis.keyFacts && aiAnalysis.keyFacts.length > 0 && (
                <div>
                  <h3 className="text-sm text-[#86868b] font-light mb-2">Key Facts</h3>
                  <ul className="space-y-1">
                    {aiAnalysis.keyFacts.map((fact: string, idx: number) => (
                      <li key={idx} className="text-gray-700 font-light text-sm flex items-start gap-2">
      ...
      {evidenceSummary && (
        <GlassCard>
          <div className="p-6">
            <h2 className="text-xl font-light text-[#1d1d1f] mb-4">Evidence Summary</h2>
            <div className="prose prose-invert max-w-none">
              <div className="text-gray-700 font-light whitespace-pre-wrap text-sm">
                ...
                <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                  ...
                  <ul className="space-y-1">
                    {evidenceSummary.contradictions.map((c: any, idx: number) => (
                      <li key={idx} className="text-gray-700 text-sm font-light">
                        • {c.contradiction} <span className="text-[#86868b]">({c.severity})</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between gap-4 mb-6">
          <div className="flex-1">
            <h1 className="text-4xl font-light text-[#1d1d1f] tracking-tight mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Dispute Summary */}
          <GlassCard>
            <h2 className="text-lg font-light text-gray-800 mb-4 tracking-wide uppercase text-xs">
              Dispute Summary
            </h2>
            <div className="space-y-3">
              <div>
                <span className="text-[#86868b] text-sm">Reason:</span>
                <span className="text-gray-800 ml-2">{dispute.reason.replace(/_/g, ' ')}</span>
              </div>
              <div>
                <span className="text-[#86868b] text-sm">Status:</span>
                <span className="text-gray-800 ml-2">{dispute.status.replace(/_/g, ' ')}</span>
              </div>
              <div>
                <span className="text-[#86868b] text-sm">Category:</span>
                <span className="text-gray-800 ml-2">{dispute.category_snapshot}</span>
              </div>
              {dispute.summary && (
                <div>
                  <span className="text-[#86868b] text-sm block mb-1">Summary:</span>
                  <p className="text-gray-700 font-light">{dispute.summary}</p>
                </div>
              )}
              ...
              {dispute.auto_triage?.decision && (
                <div className="pt-3 border-t border-gray-200">
                  <div className="text-[#86868b] text-sm mb-1">Auto-triage:</div>
                  <div className="text-gray-700 text-sm">{dispute.auto_triage.decision}</div>
                  <div className="text-[#86868b] text-xs mt-1">{dispute.auto_triage.rationale}</div>
                </div>
              )}
            </div>
          </GlassCard>
          ...
          {evidence && evidence.length > 0 && (
            <GlassCard>
              ...
              <div className="space-y-3">
                {evidence.map((ev: any) => (
                  <div key={ev.id} className="p-3 rounded-xl bg-gray-50 border border-gray-200">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-gray-800 text-sm">{ev.type}</span>
                      <span className="text-[#86868b] text-xs">
                        {ev.uploader_role} • {new Date(ev.created_at).toLocaleDateString()}
                      </span>
                    </div>
                    {ev.text_content && (
                      <p className="text-gray-600 text-sm font-light mt-2">{ev.text_content}</p>
                    )}
                    {ev.storage_path && (
                      <div className="flex gap-2 mt-2">
                        ...
                        <a
                          ...
                          rel="noopener noreferrer"
                          download
                          className="text-[#86868b] hover:text-gray-600 text-sm font-light transition-colors"
                        >
                          Download
                        </a>
                      </div>
            ...
            <GlassCard>
              ...
              {deliveryProof.delivery && (
                <div className="space-y-2 mb-4">
                  <div className="text-[#86868b] text-sm">File: {deliveryProof.delivery.file_name}</div>
                  ...
                  {deliveryProof.views && deliveryProof.views.length > 0 && (
                    <div className="pt-2 border-t border-gray-200">
                      <div className="text-[#86868b] text-sm mb-1">View Activity:</div>
                      {deliveryProof.views.map((view: any) => (
                        <div key={view.id} className="text-gray-600 text-xs">
              ...
              {deliveryProof.transfer && (
                <div className="space-y-2">
                  <div className="text-[#86868b] text-sm">Provider: {deliveryProof.transfer.provider}</div>
                  <div className="text-[#86868b] text-sm">Status: {deliveryProof.transfer.status}</div>
            ...
            <GlassCard>
              ...
              <div className="space-y-2">
                {actions.map((action: any) => (
                  <div key={action.id} className="p-2 rounded bg-gray-50 text-gray-600 text-sm">
                    ...
                    {action.note && (
                      <p className="text-[#86868b] text-xs mt-1">{action.note}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6">
          <GlassCard>
            ...
            <div className="space-y-3">
              ...
              {!['submitted', 'needs_info', 'under_review'].includes(dispute.status) && (
                <div className="space-y-2">
                  <div className="text-[#86868b] text-sm font-light mb-3">
                    Dispute Status: <span className="text-gray-800 font-medium">{dispute.status.replace(/_/g, ' ')}</span>
                  </div>
                  ...
                  <div className="text-[#86868b] text-xs font-light mt-3">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <div className="ml-2 flex items-center gap-2">
                  <span className="text-gray-800">{rift?.buyer?.email}</span>
              ...
              <div>
                <span className="text-[#86868b]">Seller:</span>
                <div className="ml-2 flex items-center gap-2">
                  <span className="text-gray-800">{rift?.seller?.email}</span>
              ...
              {caseData.dispute?.openedByUser && (
                <div>
                  <span className="text-[#86868b]">Opened By:</span>
                  <div className="ml-2 flex items-center gap-2">
                    <span className="text-gray-800">{caseData.dispute.openedByUser.email}</span>
              ...
              <div>
                ...
                <span className="text-gray-800 ml-2">{rift?.status}</span>
              </div>
            </div>
          </GlassCard>
        </div>
