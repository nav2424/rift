...
export async function POST(
  ...
  try {
    ...
    const hasDeliveryInfo = 
      ...
      (escrow.itemType === 'TICKETS' && escrow.transferMethod) ||
      (escrow.itemType === 'SERVICES' && escrow.serviceDate)

    if (!hasDeliveryInfo) {
      return NextResponse.json(
        { 
          error: `Missing delivery information. ${escrow.itemType === 'DIGITAL' ? 'Download link' : escrow.itemType === 'TICKETS' ? 'Transfer method' : 'Service date'} is required.` 
        },
        { status: 400 }
      )
    }
