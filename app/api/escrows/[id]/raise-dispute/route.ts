...
export async function POST(
  ...
  try {
    ...
    if (!['IN_TRANSIT', 'DELIVERED_PENDING_RELEASE'].includes(escrow.status)) {
      return NextResponse.json(
    ...
    if (escrow.autoReleaseScheduled) {
    ...
    if (!canTransition(escrow.status, 'DISPUTED', userRole)) {
      return NextResponse.json(
    ...
    const escrowWithUsers = await prisma.escrowTransaction.findUnique({
      where: { id },
      include: {
        buyer: true,
        seller: true,
